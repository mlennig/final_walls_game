...
Rule::Rule(){
    ...
    for (int k = 0; k < 9; k++)
        if (k == 4)
            ...
            if (hasHeading)
                mask[k] = conditionsWithoutHeadings[rand() % 5];
            else {
                mask[k] = conditions[rand() % 9];
                if(mask[k] <= 4)
                    hasHeading = true;
            }
        }
...
Strategy::Strategy(int n){
        ...
        Condition maskA[maskSize * maskSize] = {edge, edge, edge, empty, north, empty, empty, empty, empty};
        rules.push_back(new Rule(maskA, Left));

        Condition maskB[maskSize*maskSize] = {empty, empty, edge, empty, north, edge, empty, empty, edge};
        rules.push_back(new Rule(maskB, Left));

        Condition maskC[maskSize * maskSize] = {empty, empty, empty, empty, north, empty, edge, edge, edge};
        rules.push_back(new Rule(maskC, Forward));

        // Trying new conditions
        Condition mask7[maskSize * maskSize] = {dontCare, edge, dontCare, dontCare, north, dontCare, dontCare, empty, dontCare};
        rules.push_back(new Rule(mask7, Left));

        Condition mask8[maskSize * maskSize] = {dontCare, dontCare, dontCare, empty, north, edge, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask8, Left));

        Condition mask1D[maskSize * maskSize] = {ownBrick, ownBrick, edge, empty, north, edge, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1D, Left));

        Condition mask1E[maskSize * maskSize] = {edge, ownBrick, ownBrick, edge, north, empty, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1E, Right));

        Condition mask1[maskSize * maskSize] = {dontCare, empty, dontCare, dontCare, north, dontCare, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1, Forward));

        Condition mask1A[maskSize * maskSize] = {dontCare, ownBrick, dontCare, dontCare, north, dontCare, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1A, Forward));

        Condition mask1B[maskSize * maskSize] = {dontCare, dontCare, dontCare, ownBrick, north, dontCare, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1B, Left));

        Condition mask1C[maskSize * maskSize] = {dontCare, dontCare, dontCare, dontCare, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask1C, Right));

        Condition mask2[maskSize*maskSize] = {dontCare, oppBrick, dontCare, dontCare, north, empty, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask2, Right));

        Condition mask3[maskSize * maskSize] = {dontCare, dontCare, dontCare, empty, north, dontCare, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask3, Left));

        Condition mask4[maskSize * maskSize] = {dontCare, dontCare, dontCare, dontCare, north, dontCare, dontCare, empty, dontCare};
        rules.push_back(new Rule(mask4, Left));

        Condition mask5[maskSize * maskSize] = {dontCare, dontCare, dontCare, dontCare, north, oppBrick, dontCare, empty, dontCare};
        rules.push_back(new Rule(mask5, Left));

        Condition mask6[maskSize * maskSize] = {dontCare, dontCare, dontCare, oppBrick, north, dontCare, dontCare, empty, dontCare};
        rules.push_back(new Rule(mask6, Right));

        // Dealing with bricks
        Condition mask9[maskSize * maskSize] = {dontCare, oppBrick, dontCare, ownBrick, north, oppBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask9, Left));

        Condition mask10[maskSize * maskSize] = {dontCare, ownBrick, dontCare, dontCare, north, dontCare, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask10, Forward));

        Condition mask11[maskSize * maskSize] = {oppBrick, oppBrick, oppBrick, oppBrick, north, oppBrick, dontCare, ownBrick, dontCare};
        rules.push_back(new Rule(mask11, Right));

        Condition mask12[maskSize * maskSize] = {edge, edge, edge, oppBrick, north, edge, dontCare, ownBrick, edge};
        rules.push_back(new Rule(mask12, Right));

        Condition mask13[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, edge, dontCare, oppBrick, edge};
        rules.push_back(new Rule(mask13, Left));

        Condition mask14[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, edge, dontCare, dontCare, edge};
        rules.push_back(new Rule(mask14, Left));

        Condition mask15[maskSize * maskSize] = {oppBrick, oppBrick, edge, ownBrick, north, edge, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask15, Left));

        Condition mask16[maskSize * maskSize] = {oppBrick, oppBrick, edge, dontCare, north, edge, dontCare, ownBrick, dontCare};
        rules.push_back(new Rule(mask16, Left));

        Condition mask17[maskSize * maskSize] = {edge, edge, edge, oppBrick, north, empty, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask17, Right));

        Condition mask18[maskSize * maskSize] = {edge, edge, edge, empty, north, oppBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask18, Left));

        Condition mask19[maskSize * maskSize] = {edge, edge, edge, edge, north, empty, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask19, Right));

        Condition mask20[maskSize * maskSize] = {edge, edge, edge, empty, north, edge, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask20, Left));

        Condition mask21[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, edge, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask21, Left));

        Condition mask22[maskSize * maskSize] = {edge, edge, edge, edge, north, ownBrick, edge, oppBrick, dontCare};
        rules.push_back(new Rule(mask22, Right));

        Condition mask23[maskSize * maskSize] = {edge, edge, edge, edge, north, ownBrick, dontCare, oppBrick, dontCare};
        rules.push_back(new Rule(mask23, Right));

        Condition mask24[maskSize * maskSize] = {oppBrick, oppBrick, oppBrick, dontCare, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask24, Right));

        Condition mask25[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, north, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask25, Left));

        Condition mask26[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, east, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask26, Left));

        Condition mask27[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, south, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask27, Left));

        Condition mask28[maskSize * maskSize] = {edge, edge, edge, ownBrick, north, west, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask28, Left));

        Condition mask29[maskSize * maskSize] = {edge, edge, edge, north, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask29, Right));

        Condition mask30[maskSize * maskSize] = {edge, edge, edge, east, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask30, Right));

        Condition mask31[maskSize * maskSize] = {edge, edge, edge, south, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask31, Right));

        Condition mask32[maskSize * maskSize] = {edge, edge, edge, west, north, ownBrick, dontCare, dontCare, dontCare};
        rules.push_back(new Rule(mask32, Right));
